...
function IPES_subnet(addr: string, mask: string) {
    const networkAddress = IPES_toLong(IPES_mask(addr, mask));
    ...
    return {
        ...
        firstAddress: numberOfAddresses <= 2
            ? IPES_fromLong(networkAddress)
            : IPES_fromLong(networkAddress + 1),
        lastAddress: numberOfAddresses <= 2
            ? IPES_fromLong(networkAddress + numberOfAddresses - 1)
            : IPES_fromLong(networkAddress + numberOfAddresses - 2),
...
function IPES_address(name?: string, family?: string) {
    ...
    const all = Object.keys(interfaces).map((nic) => {
        ...
        const addresses = interfaces[nic].filter((details) => {
            ...
            return name === 'public' ? IPES_isPrivate(details.address)
                : IPES_isPublic(details.address);
        });
...
export {
    IPES_ipv4Regex as ipv4Regex,
    IPES_ipv6Regex as ipv6Regex,
    IPES_isV4Format as isV4Format,
    IPES_isV6Format as isV6Format,
    IPES_toBuffer as toBuffer,
    IPES_toString as toString,
    IPES__normalizeFamily as _normalizeFamily,
    IPES_fromPrefixLen as fromPrefixLen,
    IPES_mask as mask,
    IPES_cidr as cidr,
    IPES_toLong as toLong,
    IPES_fromLong as fromLong,
    IPES_subnet as subnet,
    IPES_cidrSubnet as cidrSubnet,
    IPES_not as not,
    IPES_or as or,
    IPES_isEqual as isEqual,
    IPES_isPrivate as isPrivate,
    IPES_isPublic as isPublic,
    IPES_isLoopback as isLoopback,
    IPES_loopback as loopback,
    IPES_address as address,
};
